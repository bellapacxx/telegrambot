...
async function showTelebirrPayment(bot: TelegramBot, chatId: number, session: any) {
  ...
  const accountBlock = "```" + escapeMarkdownV2(account) + "```";

  // Second code block: instructions
  const instructions = `1. ከላይ ባለው የቴሌብር አካውንት ${amount}ብር ያስገቡ
...
ዲፖዚት ስታረጉ ቦቱ ከሚያገናኛቹ ኤጀንት ዉጪ ወደ ሌላ ኤጀንት ብር ከላካቹ ቦቱ 2% ቆርጦ ይልክላችኋል\.`; // escaped `:` and `(` `)`

  const instructionsBlock = "```" + escapeMarkdownV2(instructions) + "```";

  // Plain text after code blocks
  const footer = `የሚያጋጥማቹ የክፍያ ችግር ካለ @Bbkkmmaa በዚ ኤጀንቱን ማዋራት ይችላሉ ወይም @beakal62 በዚ ሳፖርት ማዉራት ይችላሉ

የከፈለችሁበትን አጭር የጹሁፍ መለክት\\(sms\\) እዚ ላይ ያስገቡት 👇👇👇`;

  const finalMessage = `${accountBlock}\n${instructionsBlock}\n${escapeMarkdownV2(footer)}`;
